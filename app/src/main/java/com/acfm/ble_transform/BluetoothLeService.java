...
/**
 * @Description:  蓝牙服务，负责在后台实现蓝牙的连接，数据的发送接受
 */
public class BluetoothLeService extends Service {
	...
	private void broadcastUpdate(final String action, int rssi)
	{
		...
		Log.d(Constants.TAG,"Service -> broadcastUpdate -> rssi:"+Integer.toString(rssi));
	...
	{
		final Intent intent = new Intent(action);
		Log.d(Constants.TAG,"Service -> broadcastUpdate -> action:"+action);
		sendBroadcast(intent);
	}
...
//                        + characteristic.getValue());
		...
		if (data != null && data.length > 0)
		{
			...
			Log.d(Constants.TAG,"Service -> broadcastUpdate -> stringBuilder:"+new String(stringBuilder));
